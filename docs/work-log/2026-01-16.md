...
### 📝 다음 작업 예정
- [ ] 커트라인/취업기업 데이터 수집 완료 후 상세 레포트 개선
...
### ✅ 랜덤 학과 추천 기능 구현
- `/api/random-majors` API 생성
- 홀랜드 유형별 키워드 매핑으로 학과 분류
- 매번 새로고침 시 다른 학과 5개 표시
- 자동 이모지 추가 (🐴, 🔬, 🤖 등)

### ✅ 전화번호 유효성 검사 강화
- 스팸 번호 차단 로직 추가
- 반복 숫자 (11111111), 연속 숫자 (12345678), 패턴 반복 (12341234) 차단
- 실제 한국 휴대폰 번호만 허용 (010, 011, 016, 017, 018, 019)

### ✅ UI 개선
- "추천학과 더보기" 폰트 크기 확대, 여백 조정
- 학교 로딩 상태 표시 ("로딩 중...", "정보 없음")

### ✅ 빌드 오류 수정
- EMOJI_MAP 중복 키 제거 ("방송" 키 중복)
